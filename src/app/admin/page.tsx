    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900 pt-16 sm:pt-20">
      <LoadingOverlay isLoading={isLoading}>
        ...
        <motion.div
          ...
          className="bg-gray-800/50 backdrop-blur-sm rounded-2xl border border-red-500/20 p-4 sm:p-6 lg:p-8 mb-6 sm:mb-8"
        >
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 mb-4">
            <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br from-red-500 to-pink-500 rounded-full flex items-center justify-center flex-shrink-0">
              <ShieldCheckIcon className="w-6 h-6 sm:w-8 sm:h-8 text-white" />
            </div>
            <div className="min-w-0">
              <h1 className="text-2xl sm:text-3xl font-bold text-white mb-1 sm:mb-2">Admin Control Panel</h1>
        ...
        <motion.div
          ...
          className="flex flex-wrap gap-1 sm:gap-2 mb-6 sm:mb-8 overflow-x-auto pb-2"
        >
          {[
        ...
        {activeTab === 'overview' && (
          ...
          >
            {[
              { label: 'Total Users', value: mockStats.totalUsers.toLocaleString(), icon: UsersIcon, color: 'blue' },
              { label: 'Active Users', value: mockStats.activeUsers.toLocaleString(), icon: BoltIcon, color: 'green' },
              { label: 'NFTs Minted', value: `${mockStats.mintedNFTs}/${mockStats.totalNFTs}`, icon: TrophyIcon, color: 'purple' },
              { label: 'Total Staked', value: mockStats.totalStaked.toLocaleString(), icon: FireIcon, color: 'yellow' },
              { label: 'Total Rewards', value: `${mockStats.totalRewards.toLocaleString()} HCAT`, icon: BanknotesIcon, color: 'pink' },
              { label: 'Game Sessions', value: mockStats.gamesSessions.toLocaleString(), icon: TrophyIcon, color: 'cyan' },
              { label: 'Swap Volume', value: `${mockStats.swapVolume} ETH`, icon: ArrowPathIcon, color: 'orange' },
              { label: 'System Health', value: '98.5%', icon: ShieldCheckIcon, color: 'green' }
            ].map((stat, index) => (
              <motion.div
                key={stat.label}
                initial={{ opacity: 0, y: 20 }}
                ...
                className="bg-gray-800/50 backdrop-blur-sm rounded-xl sm:rounded-2xl border border-purple-500/20 p-4 sm:p-6"
              >
                <div className="flex items-center justify-between mb-3 sm:mb-4">
                  <div className={`w-10 h-10 sm:w-12 sm:h-12 bg-${stat.color}-500/20 rounded-lg flex items-center justify-center`}>
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                <input
                  ...
                  className="px-3 sm:px-4 py-2 bg-gray-700/50 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-purple-500 text-sm sm:text-base"
                />
                <LoadingButton
                  className="whitespace-nowrap px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg font-medium transition-colors text-sm"
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                          user.tier === 'Diamond' ? 'bg-pink-500/20 text-pink-400' :
        ...
        {activeTab === 'rewards' && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.2 }}
            className="bg-gray-800/50 backdrop-blur-sm rounded-2xl border border-purple-500/20 p-6"
          >
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-white">Reward System Management</h2>
              <LoadingButton
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${
                      reward.active ? 'bg-green-500/20' : 'bg-gray-500/20'
                    }`}>
                      <GiftIcon className={`w-6 h-6 ${reward.active ? 'text-green-400' : 'text-gray-400'}`} />
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <div className="text-yellow-400 font-semibold">{reward.amount} HCAT</div>
                      <div className={`text-xs ${reward.active ? 'text-green-400' : 'text-red-400'}`}>
        ...
        {activeTab === 'system' && (
          ...
          >
            <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl sm:rounded-2xl border border-purple-500/20 p-4 sm:p-6">
              <h2 className="text-xl sm:text-2xl font-bold text-white mb-4 sm:mb-6">System Status & Controls</h2>
              
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                {Object.entries(systemControls).map(([system, data]) => (
                  <div key={system} className="bg-gray-700/30 rounded-xl p-3 sm:p-4">
                    <div className="flex items-center justify-between mb-2 sm:mb-3">
                      ...
                      <div className={`flex items-center gap-1 ${getStatusColor(data.status)}`}>
                        ...
                        <span className="text-xs sm:text-sm font-medium capitalize">{data.status}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <LoadingButton
                        className="px-2 sm:px-3 py-2 text-gray-400 hover:text-white hover:bg-gray-700/50 rounded-lg transition-colors"
            ...
            {/* Emergency Controls */}
            <div className="bg-red-900/20 backdrop-blur-sm rounded-xl sm:rounded-2xl border border-red-500/20 p-4 sm:p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                <LoadingButton
                  className="border border-red-500 text-red-400 hover:bg-red-500/10 text-sm sm:text-base px-4 py-2 rounded-lg font-medium transition-colors"
                ...
                <LoadingButton
                  className="border border-yellow-500 text-yellow-400 hover:bg-yellow-500/10 text-sm sm:text-base px-4 py-2 rounded-lg font-medium transition-colors"
                ...
                <LoadingButton
                  className="border border-orange-500 text-orange-400 hover:bg-orange-500/10 text-sm sm:text-base px-4 py-2 rounded-lg font-medium transition-colors"
        ...
        {selectedUser && (
          ...
          >
            <motion.div
              ...
              className="bg-gray-800 rounded-xl sm:rounded-2xl border border-purple-500/20 p-4 sm:p-6 max-w-md w-full max-h-[90vh] overflow-y-auto"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex justify-between items-start mb-3 sm:mb-4">
                <h3 className="text-lg sm:text-xl font-bold text-white">User Details</h3>
                <button
                  onClick={() => setSelectedUser(null)}
              ...
              <div className="space-y-2 sm:space-y-3 mb-4 sm:mb-6">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-white text-sm sm:text-base">{selectedUser.nfts}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400 text-sm">Currently Staked</span>
                  <span className="text-white text-sm sm:text-base">{selectedUser.staked}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400 text-sm">Tier</span>
                  <span className="text-white text-sm sm:text-base">{selectedUser.tier}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400 text-sm">Status</span>
                  <span className={`text-sm sm:text-base ${selectedUser.status === 'active' ? 'text-green-400' : 'text-red-400'}`}>
                    {selectedUser.status}
                  </span>
                </div>
                <div className="flex justify-between">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <LoadingButton
                  className="flex-1 text-sm sm:text-base border border-cyan-500 text-cyan-400 hover:bg-cyan-500/10 px-4 py-2 rounded-lg font-medium transition-colors"
