...
import { DOT_HYPE_RESOLVER_ABI, namehash } from "@/contracts/abis";
  ...
  const fetchPrimaryDomain = useCallback(async () => {
    ...
    try {
      ...
      const domainName = await publicClient.readContract({
        ...
        functionName: "name",
        args: [namehash(address)], // Assuming namehash is used to convert address to node
      });
    ...
    const fetchAvatar = async () => {
      ...
      try {
        ...
        const avatarValue = (await publicClient.readContract({
          ...
          abi: DOT_HYPE_RESOLVER_ABI,
          functionName: "text",
          args: [namehash(address), "avatar"], // Assuming namehash is used to convert address to node
        })) as string;
